...
def transcribe_full_audio(audio_file, model_dir="whisper-generic-speakers"):
    ...
    print(f"🔄 Transcribing in high-quality chunks...")
...
def merge_overlapping_transcriptions(transcriptions):
...
def identify_speakers_in_transcription(transcription, audio, sr):
    print(f"🎭 Identifying speakers in transcription...")
...
def map_speakers_to_transcription(transcription, segments, speaker_labels):
    print(f"🎯 Mapping speakers to transcription...")
...
def find_segment_for_time(target_time, segments):
...
def add_single_speaker(transcription):
    return f"[Speaker_1]: {transcription}"
...
def hybrid_transcribe_audio(audio_file, model_dir="whisper-generic-speakers"):
    print(f"\n🎵 HYBRID TRANSCRIPTION: {Path(audio_file).name}")
    print("=" * 60)
    ...
    print(f"📝 Words: {len(transcription.split())} (raw) → {len(' '.join(speaker_transcription.split()[1::2]).split())} (speaker-aware)")
...
def main():
    print("🎵 HYBRID TRANSCRIPTION SYSTEM")
    print("=" * 60)
    print("Approach: High-quality transcription → Speaker identification")
    print("=" * 60)
    
    audio_files = [
        r"C:\Users\boudy\Downloads\drive-download-20250914T081111Z-1-001\102711-o-61466149663-816.wav",
        r"C:\Users\boudy\Downloads\091452-i-834-836.wav"
    ]
    ...
    for audio_file in available_files:
        try:
            result = hybrid_transcribe_audio(audio_file)
            
            audio_name = Path(audio_file).stem
            output_file = f"hybrid_result_{audio_name}.txt"
            ...
            print(f"\n📋 PREVIEW - {Path(audio_file).name}:")
            print(f"Raw: {len(result['raw_transcription'].split())} words")
            speaker_words = len(' '.join([line.split(']: ', 1)[1] for line in result['speaker_transcription'].split('\n') if ']: ' in line]).split())
            print(f"Speaker-aware: {speaker_words} words")
            print()
