...
def transcribe_full_audio(audio_file: str, model_dir: str = None):
    ...
    try:
        ...
        print(f"✅ Audio loaded successfully: {sr}Hz, {len(audio)} samples")
        
        # Check if it's GSM-compressed by trying to get info
        try:
            info = sf.info(audio_file)
            if info.subtype == 'GSM610':
                print("⚠️ Detected GSM-compressed WAV file. Converting to PCM...")
                # Convert GSM to PCM temporarily
                temp_dir = Path("temp_audio")
                temp_dir.mkdir(exist_ok=True)
                temp_file = temp_dir / f"converted_{Path(audio_file).stem}.wav"
                
                # Write as PCM WAV file
                sf.write(temp_file, audio, sr, format='WAV', subtype='PCM_16')
                
                # Load the converted file
                audio, sr = sf.read(temp_file)
                # Clean up temporary file
                os.unlink(temp_file)
                print("✅ GSM conversion completed")
        except Exception as info_error:
            print(f"Could not get file info: {info_error}")
            # Continue with the audio we already loaded
